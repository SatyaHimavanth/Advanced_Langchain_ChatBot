...
class UserScopedFilesystemBackend(BackendProtocol):
    ...
    def __init__(
        ...
        *,
        ...
        self._thread_key    = thread_id_key
        self._default_tenant = default_tenant
        self._default_user  = default_user
        self._default_thread = default_thread
        self._max_cached    = max_cached_users

        # LRU cache: (tenant_id, user_id, thread_id) → FilesystemBackend
        ...
        self._backends: dict[tuple[str, str, str], FilesystemBackend] = {}
    ...
    def _current_key(self) -> tuple[str, str, str]:
        """
        Extract (tenant_id, user_id, thread_id) from the current LangGraph
        invocation config.  Called at the start of every BackendProtocol method
        so the right thread workspace is always selected, even when the same
        backend instance serves concurrent requests.
        """
        cfg          = _get_config()
        ...
        thread_id    = str(configurable.get(self._thread_key, self._default_thread) or self._default_thread)
    ...
    def _get_backend(self) -> FilesystemBackend:
        ...
        if len(self._backends) >= self._max_cached:
            ...
            logger.debug(
                "UserScopedFilesystemBackend: LRU eviction for tenant=%s user=%s thread=%s",
                *evicted,
            )

        tenant_id, user_id, thread_id = key
        root_dir = (self._base / tenant_id / user_id / thread_id).resolve()
        ...
        logger.info(
            "UserScopedFilesystemBackend: new backend  tenant=%s  user=%s  thread=%s  root=%s",
            tenant_id, user_id, thread_id, root_dir,
        )
        return backend
    ...
    def user_root(
        ...
        """
        Return the on-disk root directory for a given (or current) thread.

            # During graph execution — resolved from config:
            root = backend.user_root()

            # Outside graph execution — pass keys explicitly:
            root = backend.user_root(tenant_id="acme", user_id="alice", thread_id="t1")
        """
        if tenant_id is None or user_id is None or thread_id is None:
            tenant_id, user_id, thread_id = self._current_key()
        return self._base / tenant_id / user_id / thread_id

    def cached_users(self) -> list[tuple[str, str, str]]:
        """Return the (tenant_id, user_id, thread_id) tuples currently cached."""
        return list(self._backends.keys())

    def evict(self, tenant_id: str, user_id: str, thread_id: str) -> None:
        """Remove a specific thread's cached backend (forces re-creation on next call)."""
        removed = self._backends.pop((tenant_id, user_id, thread_id), None)
        if removed:
            logger.info(
                "UserScopedFilesystemBackend: manually evicted tenant=%s user=%s thread=%s",
...
def create_backend(
    *,
    ...
    def _ns_configurable(rt: Any) -> dict:
        """Resolve configurable dict from the runtime, version-defensively."""
        cfg = getattr(rt, "config", None)
        if isinstance(cfg, dict):
            return cfg.get("configurable", {}) or {}
        # Fall back to the langgraph get_config() context var.
        ctx = _get_config()
        return ctx.get("configurable", {}) if isinstance(ctx, dict) else {}

    persistent_backend = StoreBackend(
        store=store,
        namespace=lambda rt: (
            "tenant",
            str(_ns_configurable(rt).get("tenant_id", "default_tenant") or "default_tenant"),
            "user",
            str(_ns_configurable(rt).get("user_id", "default_user") or "default_user"),
        ),
    )
    ...
    # The disk-backed per-thread filesystem is the DEFAULT backend, so the agent
    # can use plain filenames (e.g. `prime.py`) — they land in the SAME directory
    # the shell runs in (no need to remember a `/workspace/` prefix). The
    # `/workspace/` route is kept as an alias to the same backend for
    # back-compat, and `/memory/` routes to the persistent store.
    backend = CompositeBackend(
    ...
    logger.info(
        ...
        "default → disk  workspace_route=%s → disk  memory_route=%s → store",
