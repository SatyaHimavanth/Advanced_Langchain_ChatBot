...
async def _render_prompt_content(
    ...
    try:
        ...
        logger.exception(
            "MCPPromptMiddleware: failed to load prompt %r from server %r",
            prompt_name, server_name,
        )
        return (
            f"[Error loading prompt '{prompt_name}' from server '{server_name}': "
...
def _build_prompt_tool(client: Any, middleware: "MCPPromptMiddleware") -> Any:
    """
    Build a `@tool`-decorated coroutine wrapping the MCP prompt-loading call.
    ...
    async def get_mcp_prompt(
        ...
        that is relevant to the current task. Returns the prompt's messages
        formatted as text that you can apply to your response or use as
        instructions. Always check the catalog for required vs optional
        arguments before calling.

        Args:
            server_name: Name of the MCP server that owns the prompt. Must
                match a server registered in the MultiServerMCPClient.
            prompt_name: Name of the prompt to load, as shown in the MCP
                Prompt Catalog section of the system prompt.
            arguments: Optional key-value arguments to render the prompt
                template. Required arguments are listed in the catalog.
        """
...
class MCPPromptMiddleware(AgentMiddleware):
    ...
    #
    # Recall is implemented via wrap_model_call so we can override the model
    # request's system_message directly. before_model returning a state dict
    # has no recognized key for "system prompt suffix" in this langchain
    # version, so suffix-style injection is silently ignored — same lesson as
    # UserMemoryMiddleware.

    async def abefore_agent(self, state: Any, runtime: Any) -> dict | None:
        """Warm the prompt catalog cache at session start (async path)."""
        await self._warm_catalog()
        return None

    def _augmented_system(self, request: Any, suffix: str) -> SystemMessage:
        """Append `suffix` to the existing system message (or create a new one)."""
        existing = ""
        ...
        joiner = "\n\n" if existing else ""
        return SystemMessage(content=existing + joiner + suffix)

    async def awrap_model_call(self, request: Any, handler: Any) -> Any:
        """Inject the MCP prompt catalog into the system prompt every turn."""
        suffix = await self._make_prompt_suffix()
        if not suffix:
            return await handler(request)
        new_request = request.override(
            system_message=self._augmented_system(request, suffix)
        )
        logger.info(
            "MCPPromptMiddleware: injected prompt catalog (%d chars)",
            len(suffix),
        )
        return await handler(new_request)

    def wrap_model_call(self, request: Any, handler: Any) -> Any:
        """Sync injection using the cached listings (no live MCP call)."""
        servers = self._server_names()
        catalog: dict[str, list[MCPPromptInfo]] = {}
        for s in servers:
            cached = self._cache.get(s)
